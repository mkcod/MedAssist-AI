...
import {
  ...
  User, Settings, Bell, ChevronRight, Activity, Menu, X, LogOut,
  Shield, Stethoscope, Users, ClipboardList, UserCircle
} from 'lucide-react'
...
const NAV_BY_ROLE = {
  [ROLES.PATIENT]: [
    ...
    { to: '/chat',         icon: MessageSquare,   label: 'AI Assistant', badge: '2' },
...
export default function Layout() {
  ...
  const notifications = [
    { msg: 'Appointment with Dr. Priya tomorrow at 10:30 AM', time: '2h ago', unread: true },
    { msg: 'Metformin refill due in 3 days', time: '5h ago', unread: true },
    { msg: 'Lab results ready: CBC report', time: '1d ago', unread: false },
  ]
        ...
        <nav className="flex-1 px-3 py-4 space-y-0.5 overflow-y-auto">
          ...
          {nav.map(({ to, icon: Icon, label, badge }) => (
            ...
            >
              ...
              <span>{label}</span>
              {badge && (
                <span className="ml-auto bg-brand-600 text-white text-[10px] font-bold rounded-full w-4 h-4 flex items-center justify-center">{badge}</span>
              )}
            </NavLink>
          ))}
        </nav>
                  ...
                  <div className="absolute right-0 top-12 w-80 bg-white rounded-2xl border border-[#e8effc] shadow-xl z-50 overflow-hidden animate-slide-up">
                    ...
                    <div className="divide-y divide-[#f0f4ff]">
                      {notifications.map((n, i) => (
                        <div key={i} className={`px-4 py-3 flex gap-3 items-start hover:bg-[#f8faff] cursor-pointer ${n.unread ? 'bg-brand-50/50' : ''}`}>

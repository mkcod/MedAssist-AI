...
import { Activity, Stethoscope, User, ClipboardList, Users, Eye, EyeOff, Shield, ChevronRight } from 'lucide-react'
...
const ROLES = [
  ...
  },
  {
    ...
    label: 'Receptionist',
    icon: ClipboardList,
    desc: 'Handle bookings, check-ins & patient directory',
    ...
    <div className="min-h-screen bg-[#f8faff] flex">
      ...
      <div className="flex-1 flex items-center justify-center p-6 lg:p-12">
        <div className="w-full max-w-[480px] space-y-8">
            ...
            <div>
              ...
              <div className="relative">
                ...
                <button
                  ...
                  className="absolute right-3 top-1/2 -translate-y-1/2 text-[#9aaec4] hover:text-[#5a6c8a]"
                >
          ...
          {/* Quick fill all roles */}
          <div className="border-t border-[#e8effc] pt-4">
            <p className="text-[11px] text-[#9aaec4] text-center mb-3 font-medium uppercase tracking-wide">Quick access — click a role above</p>
            <div className="grid grid-cols-2 gap-2">
              {ROLES.map(r => (
                <button key={r.key} onClick={() => selectRole(r)}
                  className="text-left px-3 py-2 rounded-xl bg-[#f8faff] hover:bg-[#f0f4ff] border border-[#e8effc] transition-colors">
                  <p className="text-xs font-semibold text-[#0f1f3d]">{r.label}</p>
                  <p className="text-[10px] text-[#9aaec4] font-mono">{r.email}</p>
                </button>
              ))}
            </div>
          </div>
